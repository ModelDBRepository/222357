...
function [i,cv] = searchclosest_fixed(x,v)
% fix for returning first value if v is less than all values in x and the
% last value of x if v is greater than all the values in x
if v<x(1)
    cv=x(1);
    i=1;
    return
