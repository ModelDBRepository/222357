% polar_plot2saver_rotator.m
% This version will set an absolute path to a folder, and B, S
% values and a net_type and gc values that allow the program to form
% filenames to find cntrl and evoked (stimulated) radii (firing rates) and
% then rotates them through an angle phi also set near the top.
date_path='';
absolute_path = '/home/tmm46/projects/VerhagenLab/theory/nrn/multi_use_matlab/polar_plot_data/X10Hz30hw/full_net_2_cols_gc_30/';
B = 200;
S = 0;
net_type = 'full_net'; % actually not necessary since in path
type_of_net = net_type;
gc = '30'; % actually not necessary since in path
prefix = ['b' num2str(B) 's' num2str(S)];
bXsY = prefix;
run_number='0';
two_methods =2;
phi = 0; pi% ;
...
% b200s0_angle.dat	      b200s0.dat
% b200s0_ctrl_ave_r.dat	      b200s0_evoked_ave_r.dat
% b200s0_ctrl_centroid_ang.dat  b200s0_evoked_centroid_ang.dat
% b200s0_ctrl_centroid_mag.dat  b200s0_evoked_centroid_mag.dat
% b200s0_ctrl.dat		      b200s0_evoked_max_FR.dat
% b200s0_ctrl_max_FR.dat	      b200s0_ste.dat
% b200s0_ctrl_ste.dat
% we will store their values in variables named:
variable_names={'angle', 'fr_evoked', ...
'ctrl_ave_r', 'evoked_ave_r', ...
'ctrl_centroid_ang', 'evoked_centroid_ang', ...
'ctrl_centroid_mag', 'evoked_centroid_mag', ...
'fr_ctrl', 'evoked_max_FR',...
'ctrl_max_FR', 'ste', ...
'ctrl_ste'};
partial_filenames={'_angle', '', ...
'_ctrl_ave_r', '_evoked_ave_r', ...
'_ctrl_centroid_ang', '_evoked_centroid_ang', ...
'_ctrl_centroid_mag', '_evoked_centroid_mag', ...
'_ctrl', '_evoked_max_FR',...
'_ctrl_max_FR', '_ste', ...
'_ctrl_ste'};
filenames={}
for i=1:length(variable_names)
    filenames{i} = [char(prefix) char(partial_filenames{i})];
    cmd = ['load ' absolute_path char(filenames{i}) '.dat;'];
    eval(cmd);
    cmd = [char(variable_names{i}) '=' char(filenames{i}) ';'];
    eval(cmd);
end
...
polar([0],[20],'w') % graphing the first point, even if white, determines the scale of the polar plot
hold on
...
%     mkdir(newSubFolder);
% end
newSubFolder = ['images_rotated/' date_path];
if ~exist(newSubFolder, 'dir')
    mkdir(newSubFolder);
end
...
saveas(fig_handle,['images_rotated/' date_path '/' type_of_net '_' bXsY '_model_polar_plot_' run_number '_' num2str(two_methods) '.fig']); % save as fig
saveas(fig_handle,['images_rotated/' date_path '/' type_of_net '_' bXsY '_model_polar_plot_' run_number '_' num2str(two_methods)],'epsc'); % eps extension auto
