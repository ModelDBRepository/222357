...
%             tce_isi_end(s_index) = i_start_next;
            [i_end_later,cv] = searchclosest_fixed(tce,SOFF(s_index - 1)+0.050);
            %alter 0.050 if you do not
            %want to examine a different spontaneous
            %window that is not 150 ms
            tce_contr_start(s_index - 1) = i_end_later;
            tce_contr_end(s_index - 1) = i_start;
...
cmd=['fid=fopen(''/Users/morse/Documents/projects/VerhagenLab/20140814dev/ORNstim/data/sort_breath/' filename '_SC50ms20140814.dat'',''w'');'];
eval(cmd)
