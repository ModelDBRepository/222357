...
response_start_delay=0.0 % start looking for spikes when the stimulus starts %%% Read into response_profile_static_var_dur 
response_window_delay=response_start_delay;

response_period_end=0.025 % <-20151027 0.05% 10 to small? % (make 80 or 100) The end of the response_period in real time, or the response window + response start delay 
response_window=response_period_end-response_start_delay % duration of the response period. 
selected_duration=.01

%% Control period Settings%%

check_time=0.015; % 20151012 for sniffs 0.09%The minimum amount of time between stimulations
contr_rp_delay=0.01;
...
fid=fopen(['data/' date_path '/shared_file_pointers.dat'],'r'); % date path makes parallel use possible although not used!
line1=fgets(fid);
fclose(fid);
...
disp(['batch_make_bi_polars_edit:avoided reloading tank which normally we would have got tank from ' cmd]);
filename = line1(6:end -8);
disp([' and hopefully it has no consequence to set filename to ' filename ]);
analyze_breathing
...
[status,message,messageid]=mkdir(['data/' date_path '/sort_breath']);

%cmd=['fid=fopen(''data/sort_breath/' filename '_20150609E.dat'',''w'');'];
cmd=['fid=fopen(''data/' date_path '/sort_breath/0model_' num2str(folder_index-1) '.dat'',''w'');']; % 0model_X format for breath info file
eval(cmd)
...
for stimid_index=1:length(stimids)
    y = [stimids(stimid_index); evoked_sc(stimid_index); ...
        ...
        contr_rp_delay;selected_duration;response_start_delay;response_period_end; check_time];%stim_angle_plus_50 is being used to save an off response window
    fprintf(fid, '%g %g %g %g %g %g %g %g %g %g %g %g %g\n', y);
end
fclose(fid);
crlf=[char(13) char(10)];
%disp(['wrote stimid, evoked spikes, stim angles file:' crlf 'data/sort_breath/' filename '_20150609D.dat']);
disp(['wrote stimid, evoked spikes, stim angles file:' crlf 'data/' date_path '/sort_breath/0model_' num2str(folder_index-1) '.dat']);

% write this into the shared_file_pointers.dat file
fid=fopen(['data/' date_path '/shared_file_pointers.dat'],'w'); % date_path makes parallel use possible
fprintf(fid, line1); % the first line that was read in at the top of the script
breath_info_filename=['data/' date_path '/sort_breath/0model_' num2str(folder_index-1) '.dat'];
fprintf(fid, [breath_info_filename '\n'])
fclose(fid);
